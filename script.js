...
async function sendLeadToCRM(data) {
  ...
  try {
    ...
    console.log('CRM/table response:', result);

    if (!response.ok || !result.success) {
      throw new Error(result.message || 'Estimate request saving failed');
    }

    console.log('Estimate request saved successfully');

  } catch (error) {
    console.error('Estimate request saving failed:', error);
  }
}

function formatLeadMessage(data) {
  return `Новая заявка на расчёт сметы — Qadam Stroi
Имя: ${data.name}
Телефон: ${data.phone}
...
let chatEstimateRequestSent = false;
let lastChatRequestPhone    = '';
...
function chatQuickReply(key) {
  ...
  setTimeout(() => {
    if (key === 'contact_q') {
      if (chatEstimateRequestSent) {
        addBotMessage('Вы уже отправили заявку на расчёт сметы. Менеджер свяжется с вами в WhatsApp.');
        return;
      }
      addBotMessage('Оставьте имя и номер — менеджер напишет вам в WhatsApp. Не будем звонить 10 раз.');
...
function submitChatContactForm(event) {
  ...
  const lead = Object.assign({}, leadData, {
    name:    nameInput.value.trim(),
    phone:   phoneInput.value.trim(),
    source:  'chat_estimate_request',
    message: 'Заявка на расчёт сметы через помощника'
  });

  chatEstimateRequestSent = true;
  lastChatRequestPhone    = lead.phone;
  ...
  addBotMessage('Заявка на расчёт сметы принята. Менеджер свяжется с вами в WhatsApp.');

  setTimeout(() => {
    addBotLink(
      'Или вы можете написать нам прямо сейчас:',
      'https://wa.me/77029727951?text=Здравствуйте,%20я%20обращался%20через%20сайт'
    );
  }, 600);
}
